...
class RNNDecoder(AbsDecoder):
    ...
    def caul_gini(self, softmax_value):
        gini = 0
...
#         logging.info(f"olength is {olength}")
    ...
    def score(self, yseq, state, x):
        # to support mutiple encoder asr mode, in single encoder mode,
        # convert torch.Tensor to List of torch.Tensor
        #print("call rnn decoder score:", yseq, state, x)
        if self.num_encs == 1:
            x = [x]
        ...
        outp = F.softmax(logits, dim=1).squeeze(0)
        c=[o for o in outp.numpy()]
        sum_output = 0
        for o in c:
            sum_output += o
#         logging.info(f"outp is {outp}, sum is {sum_output}")
#         logging.info(f"z_list is {z_list}")
        return (
            logp,
            dict(
                c_prev=c_list[:],
