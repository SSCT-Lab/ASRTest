...
retrain_model= # retrain a model from Model Zoo and use it for decoding.
...
Options:
    ...
    --retrain_model      # Download a model from Model Zoo and use it for decoding (default="${retrain_model}").
...
if [ -n "${retrain_model}" ]; then
    log "Use ${retrain_model} for decoding and evaluation"
    asr_exp="${expdir}/${retrain_model}"
    #asr_exp=`echo ${asr_exp} | sed 's/ //g'`
    mkdir -p "${asr_exp}"
    
    retrain_exp="${expdir}/${retrain_model}_${train_set}"
    retrain_exp=`echo ${retrain_exp} | sed 's/ //g'`
    mkdir -p "${retrain_exp}"
    
    # If the model already exists, you can skip downloading
    #espnet_model_zoo_download --unpack true "${retrain_model}" > "${asr_exp}/config.txt"

    # Get the path of each file
    config_txt=$(find ${asr_exp} -name "config.txt" | head -n 1)
    if [ -z "${config_txt}" ]; then
        _asr_model_file=$(find ${asr_exp} -name "valid.acc.ave.pth" | head -n 1)
        _asr_train_config=$(find ${asr_exp} -name "config.yaml" | head -n 1)
    else
    ...
    retrain_config=${retrain_exp}/config.yaml
    
#     cp ${_asr_model_file} "${retrain_exp}/valid.acc.ave_retrain.pth"
#     cp ${_asr_train_config} "${retrain_exp}/config.yaml"
#     echo "${_asr_model_file}"
    
#     chmod 777 "${retrain_exp}/valid.acc.ave_retrain.pth"
#     chmod 777 "${retrain_exp}/config.yaml"
    
#     retrain_model_file= ${retrain_exp}/valid.acc.ave_retrain.pth
...
#     inference_asr_model=$(basename "${_asr_model_file}")
    #inference_asr_model=$(basename "${retrain_exp}/valid.acc.ave_retrain.pth")
...
#         download_model=`echo ${download_model} | sed 's/ //g'`
...
if ! "${skip_data_prep}"; then
    if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
        ...
        local/retrain_data.sh ${local_data_opts}
    fi
...
if ! "${skip_train}"; then
 ...
 if [ ${stage} -le 9 ] && [ ${stop_stage} -ge 9 ]; then
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        log "Generate '${retrain_exp}/run.sh'. You can resume the process from stage 10 using this script"
        mkdir -p "${retrain_exp}"; echo "${run_args} --stage 10 \"\$@\"; exit \$?" > "${retrain_exp}/run.sh"; chmod +x "${retrain_exp}/run.sh"

        # NOTE(kamo): --fold_length is used only if --batch_type=folded and it's ignored in the other case
        log "ASR training started... log: '${retrain_exp}/train.log'"
        if echo "${cuda_cmd}" | grep -e queue.pl -e queue-freegpu.pl &> /dev/null; then
            # SGE can't include "/" in a job name
            jobname="$(basename ${retrain_exp})"
        else
            jobname="${retrain_exp}/train.log"
        fi

        # shellcheck disable=SC2086
        ${python} -m espnet2.bin.launch \
            --cmd "${cuda_cmd} --name ${jobname}" \
            --log "${retrain_exp}"/train.log \
            --ngpu "${ngpu}" \
            --num_nodes "${num_nodes}" \
            --init_file_prefix "${retrain_exp}"/.dist_init_ \
            --multiprocessing_distributed true -- \
            ${python} -m espnet2.bin.asr_train \
                --use_preprocessor true \
                --bpemodel "${bpemodel}" \
                ...
                --output_dir "${retrain_exp}" \
                --init_param "${_asr_model_file}" \
                ${_opts} ${asr_args}
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        log "Stage 11: Decoding: training_dir=${retrain_exp}"
        ...
        # 2. Generate run.sh
        log "Generate '${retrain_exp}/${inference_tag}/run.sh'. You can resume the process from stage 11 using this script"
        mkdir -p "${retrain_exp}/${inference_tag}"; echo "${run_args} --stage 11 \"\$@\"; exit \$?" > "${retrain_exp}/${inference_tag}/run.sh"; chmod +x "${retrain_exp}/${inference_tag}/run.sh"

        for dset in ${test_sets}; do
            _data="${data_feats}/${dset}"
            _dir="${retrain_exp}/${inference_tag}/${dset}"
            _logdir="${_dir}/logdir"
            mkdir -p "${_logdir}"
            ...
            ${_cmd} --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/asr_inference.JOB.log \
                ${python} -m espnet2.bin.asr_test \
                    ...
                    --asr_train_config "${retrain_config}" \
                    --asr_model_file "${retrain_exp}/${inference_asr_model}" \
    ...
    if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ]; then
        ...
        [ -f local/score.sh ] && local/score.sh ${local_score_opts} "${retrain_exp}"

        # Show results in Markdown syntax
        scripts/utils/show_asr_result.sh "${retrain_exp}" > "${retrain_exp}"/RESULTS.md
        cat "${retrain_exp}"/RESULTS.md
