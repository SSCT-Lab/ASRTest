...
if [ -z "${train_config}" ]; then
    #download_models
    #train_config=$(find ${download_dir}/${models} -name "train*.yaml" | head -n 1)
    train_config=decode/download/tedlium3.conformer/conf/tuning/train_conformer.yaml
fi
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    #local/prepare_train_data.sh
    utils/fix_data_dir.sh data/${retrain_set}.orig
    utils/data/modify_speaker_info.sh --seconds-per-spk-max 2000 data/${retrain_set}.orig data/${retrain_set}
    utils/fix_data_dir.sh data/${retrain_dev}.orig
    utils/data/modify_speaker_info.sh --seconds-per-spk-max 2000 data/${retrain_dev}.orig data/${retrain_dev}
#     for dset in ${retrain_set} ${retrain_dev}; do
#         #utils/fix_data_dir.sh data/${dset}.orig
#         utils/data/modify_speaker_info.sh --seconds-per-spk-max 180 data/${dset}.orig data/${dset}
#     done
fi

feat_tr_dir=${dumpdir}/${retrain_set}/delta${do_delta}; mkdir -p ${feat_tr_dir}
feat_dt_dir=${dumpdir}/${retrain_dev}/delta${do_delta}; mkdir -p ${feat_dt_dir}
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for x in ${retrain_set} ${retrain_dev}; do
...
#     # compute global CMVN
#     compute-cmvn-stats scp:data/${retrain_set}/feats.scp data/${retrain_set}/cmvn.ark

    # dump features for training
    dump.sh --cmd "$train_cmd" --nj 32 --do_delta ${do_delta} \
        data/${retrain_set}/feats.scp ${cmvn} exp/dump_feats/${retrain_set} ${feat_tr_dir}
    dump.sh --cmd "$train_cmd" --nj 32 --do_delta ${do_delta} \
        data/${retrain_dev}/feats.scp ${cmvn} exp/dump_feats/${retrain_dev} ${feat_dt_dir}
...
#     wc -l ${dict}
    ...
    data2json.sh --feat ${feat_tr_dir}/feats.scp --bpecode ${bpemodel}.model \
         data/${retrain_set} ${dict} > ${feat_tr_dir}/data_${bpemode}${nbpe}.json
    data2json.sh --feat ${feat_dt_dir}/feats.scp --bpecode ${bpemodel}.model \
         data/${retrain_dev} ${dict} > ${feat_dt_dir}/data_${bpemode}${nbpe}.json
#     for rtask in ${recog_set}; do
...
if [ -z ${tag} ]; then
    expname=${retrain_set}_${backend}_$(basename ${train_config%.*})
    if ${do_delta}; then
    ...
    expname=${retrain_set}_${backend}_${tag}
fi

# expdir=exp/${expname}
expdir=exp/${retrain_set}_${p}
mkdir -p ${expdir}
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    echo "stage 4: Network Training, recog_model is ${recog_model}"
    ${cuda_cmd} --gpu ${ngpu} ${expdir}/train.log \
        ...
        --epochs 20 \
        --enc-init ${recog_model} \
        --dec-init ${recog_model} \
        --enc-init-mods 'encoder.' \
        --dec-init-mods 'decoder.,ctc.'
fi
